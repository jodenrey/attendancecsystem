            ...
            <div class="col-lg-12">
              ...
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-info">View Student Attendance</h6>
                    <?php echo $statusMsg; ?>
                </div>
                <div class="card-body">
                  <form method="post">
                    <div class="form-group row mb-3">
                        ...
                        if ($_SESSION['userType'] == 'Parent') {
                          ...
                          foreach ($studentChildren as $student) {
                            $qry = "SELECT * FROM tblstudents WHERE admissionNumber = '$student' ORDER BY firstName ASC";
                          ...
                          $qry = "SELECT * FROM tblstudents WHERE admissionNumber = '$_SESSION[admissionNumber]'";
                          $result = $conn->query($qry);
                          $rows = $result->fetch_assoc();
                          echo '<input type="hidden" name="admissionNumber" value="' . $rows['admissionNumber'] . '">';
                          echo '<p class="form-control mb-3">' . $rows['firstName'] . ' ' . $rows['lastName'] . '</p>';
                        } else {
                    ...
                    <button type="submit" name="view" class="btn btn-info">View Attendance</button>
                  </form>
                </div>
              </div>
              ...
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-info">Class Attendance</h6>
                </div>
                      ...
                      if($num > 0) { 
                        while ($rows = $rs->fetch_assoc()) {
                            ...
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($rows['otherName']); ?></td>
                                <?php if ($_SESSION['userType'] != 'Parent') { ?>
                                    <td><?php echo htmlspecialchars($rows['admissionNumber']); ?></td>
                                    <td><?php echo htmlspecialchars($rows['className']); ?></td>
                                    <td><?php echo htmlspecialchars($rows['classArmName']); ?></td>
                                    <td><?php echo htmlspecialchars($rows['sessionName']); ?></td>
                                    <td><?php echo htmlspecialchars($rows['termName']); ?></td>
                                <?php } ?>
                                <td style="background-color: <?php echo $colour; ?>"><?php echo $status; ?></td>
                                <td><?php echo htmlspecialchars($rows['dateTimeTaken']); ?></td>
                            </tr>
                            <?php
                        }
                    }
